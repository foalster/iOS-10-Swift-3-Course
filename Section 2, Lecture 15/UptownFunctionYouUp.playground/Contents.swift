...
var sigourneyWeaverAlienStomperShoes = 580.00

func purchaseItem(currentBalance: inout Double, itemPrice: Double){
    if currentBalance > 0.0{
        if itemPrice <= currentBalance {
        currentBalance = currentBalance - itemPrice
        print("Purchased item for: $\(itemPrice) and your current balance is $\(currentBalance)")
    } else {
        print("You don't have enough money to buy this. You only have $\(currentBalance) and what you want to buy costs $\(itemPrice)")
        }
    }else {
